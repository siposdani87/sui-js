...
SUI.Table.prototype._initSearch = function() {
  if (this.options.columns[this.options.columns.length - 1] === 'search') {
    ...
    const subLabelNode = new SUI.Item('label');
...
SUI.Table.prototype._renderHeader = function(headerNode, columnIndex) {
  ...
  if (headerTitle || headerDesc) {
    const iconInfo = new SUI.Item('em');
    if (headerTitle) {
...
SUI.Table.prototype._initStructure = function() {
  this.tbody = new SUI.Item('tbody');
  this.tableNode.appendChild(this.tbody);

  this.tfoot = new SUI.Item('tfoot');
  this.tableNode.appendChild(this.tfoot);

  const footerRow = new SUI.Item('tr');

  const statisticsNode = new SUI.Item('td');
  statisticsNode.addClass('pager-statistics');
  footerRow.appendChild(statisticsNode);

  const pagerNode = new SUI.Item('td');
...
SUI.Table.prototype._addHeaderRow = function(item, rowIndex) {
  const headerRow = new SUI.Item('tr');
  ...
  const headerNameCell = new SUI.Item('td');
  ...
  const headerActionCell = new SUI.Item('td');
...
SUI.Table.prototype._renderDataNodeByItem = function(item, rowIndex, column, parentNode) {
  ...
  SUI.eachArray(items, (item) => {
    if (!SUI.instanceOf(item, SUI.Item) ) {
      const dataNode = new SUI.Item('span');
      dataNode.setHtml(/** @type {string} */(item));
      item = dataNode;
    }
    parentNode.appendChild(/** @type {!SUI.Item} */ (item));
    if (item.getAttribute('title') || item.getAttribute('desc')) {
      const tooltip = new SUI.Tooltip(/** @type {!SUI.Item} */ (item));
      tooltip.render();
    }
  });
};
...
SUI.Table.prototype._renderDataNode = function(tableDataNode, item, rowIndex, column, columnIndex) {
  if (SUI.inArray(['search', 'actions'], column)) {
    ...
    const labelNode = new SUI.Item('span');
...
SUI.Table.prototype._renderActions = function(tableDataNode, item) {
  const containerNode = new SUI.Item('div');
