...
SUI.GoogleMap.prototype.createPolygon = function(id, title, points, opt_polygonData = {}, opt_options = {}) {
  const polygonData = new SUI.Objekt(opt_polygonData);
  if (!polygonData.get('id')) {
    polygonData.set('id', id);
  }
  const options = new SUI.Objekt(this.polygonOptions);
...
SUI.GoogleMap.prototype._cleanPolygonData = function(polygonData) {
  const cleanData = new SUI.Objekt();
...
SUI.GoogleMap.prototype.setMarkers = function(opt_options = {}) {
  this.markers = /** @type {!SUI.Collection<!SUI.Objekt>} */ (new SUI.Collection());

  this.markerOptions = new SUI.Objekt({
    draggable: false,
  });
...
SUI.GoogleMap.prototype.setHeatmap = function(opt_options = {}) {
  ...
  this.heatmapOptions = new SUI.Objekt({
    opacity: 0.6,
    radius: null,
    gradient: gradient,
  });
...
SUI.GoogleMap.prototype.setPolygons = function(opt_options = {}) {
  this.polygons = /** @type {!SUI.Collection<!SUI.Objekt>} */ (new SUI.Collection());

  this.polygonOptions = new SUI.Objekt({
    geodesic: true,
    strokeColor: '#FF0000',
    strokeOpacity: 0.5,
    strokeWeight: 2,
...
SUI.GoogleMap.prototype.createMarker = function(id, title, iconName, latitude, longitude, opt_markerData = {}, opt_options = {}) {
  const markerData = new SUI.Objekt(opt_markerData);
  if (!markerData.get('id')) {
    markerData.set('id', id);
  }
  const options = new SUI.Objekt(this.markerOptions);
  options.merge(opt_options);
