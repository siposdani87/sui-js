import { typeCast, each } from "../base";
 ...
 */
export const Cookie = function(opt_options = {}) {
 ...
 * @return {undefined}
 */
Cookie.prototype._setOptions = function(opt_options = {}) {
  const _self = this;
 ...
 * @return {string}
 */
Cookie.prototype._getPropertyName = function(name) {
 ...
 * @return {string}
 */
Cookie.prototype._getName = function(propertyName) {
 ...
 * @return {undefined}
 */
Cookie.prototype.set = function(name, value, opt_expires = '', opt_path = '/', opt_domain = '', opt_secure = false) {
  ...
  if (opt_expires) {
    switch (opt_expires.constructor) {
      case Number:
        opt_expires = (opt_expires as any) === Infinity ? '; expires=Fri, 31 Dec 9999 23:59:59 GMT' : '; max-age=' + (/** @type {number} */ (opt_expires as any) * 60 * 60);
        break;
      case Date:
        opt_expires = '; expires=' + (opt_expires as any).toUTCString();
        break;
      default:
        break;
    }
 ...
 * @return {undefined}
 */
Cookie.prototype.remove = function(name, opt_path = '', opt_domain = '', opt_secure = false) {
  if (this.hasKey(name)) {
 ...
 * @return {boolean}
 */
Cookie.prototype.hasKey = function(name) {
  const propertyName = this._getPropertyName(name);
