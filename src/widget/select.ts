import { isArray, eq, format, each, neq, inArray, clear, remove, generateId, mdl } from "../base";
 ...
 */
export const Select = function(input, label, error, inputBlock) {
  BaseWidget.call(this, input, label, error, inputBlock);
  this._init();
};
Select.prototype = Object.create(BaseWidget.prototype);
 ...
 * @return {undefined}
 */
Select.prototype._initPopup = function() {
 ...
 * @return {undefined}
 */
Select.prototype._initChangeEvent = function() {
 ...
 * @return {undefined}
 */
Select.prototype._initOptions = function() {
 ...
 * @return {*}
 */
Select.prototype.getOptionValue = function(opt_attribute) {
  const value = this.getValue();
  if (value) {
    const option = this.options.findById(value);
    return opt_attribute ? option.get(format('item.{0}', [opt_attribute])) : option;
  }
  return value;
};
 ...
 * @return {undefined}
 */
Select.prototype.showLoader = function() {
 ...
 * @return {undefined}
 */
Select.prototype._hideLoader = function() {
  this.iconNode.setHtml('expand_more');
 ...
 * @return {undefined}
 */
Select.prototype.setOptions = function(items, opt_value = 'value', opt_name = 'name', opt_image = '') {
  const optionNodes = new Query('option', this.input);
  optionNodes.each((optionNode) => {
 ...
 * @return {undefined}
 */
Select.prototype._setSelectTags = function(ids) {
 ...
 * @private
 */
Select.prototype._setSimpleTag = function(id) {
 ...
 * @private
 */
Select.prototype._setMultipleTag = function(ids) {
  const options = [];
  each(ids, (id) => {
    const option = this.options.findById(id);
    if (option) {
 ...
 */
Select.prototype._setTags = function(tags) {
  if (!isArray(tags)) {
    tags = [tags];
  }
 ...
 * @return {undefined}
 */
Select.prototype._setSelectedIds = function(ids) {
 ...
 * @return {!Array}
 */
Select.prototype._getSelectedIds = function() {
  const ids = [];
  this.options.each(function(option) {
    const optionNode = option.get('option_node');
    const node = optionNode.getNode();
 ...
 * @private
 */
Select.prototype._handleSelectedId = function(id) {
  let ids = this._getSelectedIds();
  if (this.isMultiple()) {
    if (eq(id, '') || eq(ids[0], '')) {
      clear(ids);
 ...
 * @return {undefined}
 */
Select.prototype._drawItems = function(items) {
 ...
 * @return {undefined}
 */
Select.prototype._drawSearchInput = function() {
 ...
 * @return {undefined}
 */
Select.prototype._search = function(query) {
  this.query = query;
