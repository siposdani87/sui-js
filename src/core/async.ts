import { each, isUndefined, isFunction, eq, copyArray, consoleWarn } from "../base";
 ...
 */
export const Async = function(opt_sum?) {
  this.sum = opt_sum || 0;
  this._clear();
};
 ...
 * @return {!Promize}
 */
Async.prototype.parallel = function(calls, opt_args) {
 ...
 * @return {undefined}
 */
Async.prototype.parallelFunction = function(call, opt_args, opt_index) {
  const index = !isUndefined(opt_index) ? /** @type {number} */ (opt_index) : this.call.counter++;
  this.call.results[index] = null;
 ...
 * @return {!Promize}
 */
Async.prototype._parallelWrapper = function(call, length, allowEvent, index, opt_args) {
  const deferred = new Deferred();
  const args = opt_args || [];
 ...
 * @return {!Promize}
 */
Async.prototype._parallelCaller = function(length, isError, result, allowEvent, index, opt_args) {
  const deferred = new Deferred();
 ...
 * @return {undefined}
 */
Async.prototype._clear = function() {
  this.call = {
    sum: 0,
    isError: false,
    counter: 0,
 ...
 * @return {undefined}
 */
Async.prototype.setStatus = function(sum, isError, counter, results) {
  this.call.sum = sum;
  this.call.isError = isError;
 ...
 * @return {undefined}
 */
Async.prototype.eventComplete = function(isError, results) {
 ...
 * @return {!Promize}
 */
Async.prototype.serial = function(calls, opt_args) {
  const deferred = new Deferred();
  if (calls.length === 0) {
    const results = opt_args || this.call.results;
    deferred.resolve(results);
 ...
 * @return {!Promize}
 */
Async.prototype._serialWrapper = function(calls, index, opt_args) {
  const deferred = new Deferred();
 ...
 * @return {!Promize}
 */
Async.prototype._serialCaller = function(calls, index, result, opt_args) {
  const deferred = new Deferred();
  this.call.results[index] = result;
